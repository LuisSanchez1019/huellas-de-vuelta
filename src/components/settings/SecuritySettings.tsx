...
/**
 * Seguridad y privacidad unificadas en una sola pantalla: cambio de
 * contraseña primero, y debajo — como una sección propia — todo lo que antes
 * vivía en «Privacidad» (preferencias opcionales, consentimiento otorgado,
 * derechos del titular y eliminación de cuenta). Un solo punto de menú.
 */
export default function SecuritySettings() {
  ...
  // Privacidad
  const [privacyState, setPrivacyState] = useState<"loading" | "ready" | "error">("loading");
    ...
    (async () => {
      ...
      try {
        ...
        try {
          ...
          // El historial de consentimientos es informativo: si falla, no bloquea la sección.
          if (active) setConsents([]);
        }
      } catch {
        if (active) setPrivacyState("error");
      }
    })();
    return () => {
      active = false;
  ...
  async function handlePasswordSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!email) {
      setPasswordError("No hay una sesión activa con correo.");
      return;
    }
    if (next.length < 8) {
    ...
    try {
      ...
      if (updateError) {
        setPasswordError(updateError.message || "No fue posible cambiar la contraseña.");
        return;
      }
      setCurrent("");
      setNext("");
  ...
  return (
    <section className={styles.wrap}>
      <header className={styles.head}>
        <h1 className={styles.title}>Seguridad y privacidad</h1>
        <p className={styles.subtitle}>
          Cambia tu contraseña y gestiona tus autorizaciones sobre el tratamiento de tus datos
          personales.
        </p>
      </header>
      ...
      {/* --- Sección Privacidad: unificada debajo del cambio de contraseña --- */}
        ...
        <>
          <div className={styles.card}>
            ...
            <p className={styles.note} style={{ marginTop: 0 }}>
              Cada opción es una autorización independiente y voluntaria. Todas están desactivadas
              por defecto y puedes retirarlas en cualquier momento. Los cambios se aplican en el
              servidor y quedan registrados con su fecha.
            </p>
          ...
          <div className={styles.card}>
            ...
            <ul className={styles.plainList}>
              ...
              <li>
                Los datos de una mascota son privados mientras esté «En casa». Al reportarla como
                perdida o publicarla en adopción se muestran su foto, nombre, especie, raza, color y
                edad — nunca datos tuyos, salvo el número que autorices arriba.
              </li>
              <li>
                La zona (ciudad y barrio) de un reporte es pública para ayudar a ubicar la mascota; no
                se pide ni se muestra la dirección exacta.
              </li>
              ...
              <ul className={styles.plainList}>
                {consents.map((c) => (
                  <li key={`${c.policyType}-${c.policyVersion}-${c.acceptedAt}`}>
                    {CONSENT_LABELS[c.policyType] ?? c.policyType} — versión {c.policyVersion}, aceptada
                    el {formatConsentDate(c.acceptedAt)}.
                  </li>
                ))}
              </ul>
            )}
            <p className={styles.note}>
              Este registro es de solo lectura y se conserva como prueba de la autorización otorgada.
              Al publicarse una nueva versión de la política, se te pedirá aceptarla sin borrar el
              historial.
            </p>
          </div>
